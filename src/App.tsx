...
const App = () => {
  ...
  const ToastComponent = () => toast && (
    <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] w-full max-w-xs transition-all duration-300">
      <div className={`p-3 rounded-xl shadow-2xl text-sm font-semibold text-gray-900 backdrop-blur-sm ${toast.color}`}>
        {toast.message}
      </div>
    </div>
  );
    ...
    const unsubscribeProfile = onSnapshot(profileRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setDoc(profileRef, initialData).then(() => {
          ...
          showToast('Welcome to SpawnEngine! Initial XP/SPN credited.', 'success');
        });
      }
      setIsLoading(false);
    }, (error) => {
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    if (!isReadyForCheckIn) {
      const timeRemainingMs = lastCheckIn.getTime() + (24 * 60 * 60 * 1000) - now.getTime();
  ...
  const handlePostCase = useCallback(async () => {
    if (!db || !userId) return showToast('Platform is initializing.', 'error');
    if (!newCaseTitle || !newCaseDesc) return showToast('Title and description are required.', 'error');
    ...
    try {
      ...
      setNewCaseTitle('');
      setNewCaseDesc('');
      showToast('Question posted successfully to SupCast Network!', 'success');
    } catch (e) {
  ...
  const streakReady = useMemo(() => {
    ...
    return (new Date().getTime() - profileData.lastCheckIn.getTime()) >= (24 * 60 * 60 * 1000);
  ...
  const HomeTab = () => {
    ...
    const oracleFeed = [
      { type: 'alpha', message: "Alpha Signal: Whale '0xab...c78' purchased 3 Launchpad Packs. Unusual activity.", icon: <ShieldCheck className="w-4 h-4 text-neon" /> },
      { type: 'gravity', message: "Gravity Shift: Creator Y cluster has +200% XP flow. Hype cycle initiating.", icon: <TrendingUp className="w-4 h-4 text-yellow-400" /> },
      { type: 'burn', message: "Burn Event: High-volume wallet '0xde...f12' burned 50% of Creator Z tokens.", icon: <Flame className="w-4 h-4 text-red-500" /> },
      { type: 'quest', message: "Quest: New IRL Quest available: Check-in at ETH London Meetup!", icon: <MapPin className="w-4 h-4 text-blue-400" /> }
    ];
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 gap-3">
          <div className={`${cardBg} ${borderColor} border p-4 rounded-xl text-center shadow-lg transition duration-200 hover:scale-[1.02] ${neonShadow}`}>
            ...
            <p className={`text-4xl font-extrabold mt-1 ${neon}`}>{profileData.xpBalance.toLocaleString()}</p>
          </div>
          <div className={`${cardBg} ${borderColor} border p-4 rounded-xl text-center shadow-lg transition duration-200 hover:scale-[1.02]`}>
            <p className="text-xs text-gray-400 uppercase tracking-wider">SPAWN TOKEN</p>
            <p className={`text-4xl font-extrabold mt-1 ${accentBlue}`}>{profileData.spawnTokenBalance.toFixed(2)}</p>
          </div>
        </div>
        ...
        <div className={`${cardBg} ${borderColor} border p-4 rounded-xl space-y-3 shadow-lg`}>
          <div className="flex justify-between items-center">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-full font-bold transition duration-300 text-sm shadow-md 
                ${streakReady ? 'bg-neon-green/30 text-neon border border-neon' : 'bg-gray-700/50 text-gray-400 border border-gray-600 cursor-not-allowed'}
              `}
            >
              {streakReady ? 'Check In Now' : 'Check In'}
            </button>
          </div>
          <p className="text-xs text-gray-400">Next check-in in: <span className="font-mono text-white">{timeRemainingText}</span></p>
          ...
          <button className="w-full text-xs text-center text-red-400/80 hover:text-red-300 pt-1">
            <ShieldCheck className="w-3 h-3 inline-block mr-1" /> Buy Streak Insurance (Pillar 3)
          </button>
        </div>
        ...
        <div className="space-y-3">
            ...
            <div className="space-y-2">
                {oracleFeed.map((item, index) => (
                    <div key={index} className="flex items-start space-x-3 p-3 bg-gray-900 rounded-xl border border-gray-800 shadow-md">
  ...
  const LootTab = () => {
    ...
    const handleSynthesis = async () => {
      if (!db || !userId) return;
      if (fragments < 3) return showToast('Not enough Fragments (3 needed).', 'error');
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-around bg-gray-900 p-1 rounded-xl border border-gray-700/50 shadow-inner">
            <button className={`w-1/3 py-2 text-sm rounded-lg ${neonBg}/30 ${neon} font-semibold transition-colors`}>Packs</button>
            <button className="w-1/3 py-2 text-sm rounded-lg text-gray-400 hover:bg-gray-800 transition-colors">Pull Lab</button>
        ...
        <div className={`${cardBg} ${borderColor} border p-4 rounded-xl space-y-3 shadow-lg`}>
            <div className="flex justify-between items-start">
                ...
                <span className="text-xs font-mono px-2 py-0.5 rounded-full bg-gray-700 text-white">ID: S001</span>
            </div>
            <p className="text-sm text-gray-400">A base pack with guaranteed Fragments and a chance for Shards. (Pillar 2)</p>
            <div className="flex justify-between items-center pt-2 border-t border-gray-800">
                <span className={`text-2xl font-extrabold ${neon}`}>{starterPack.count} available</span>
                <button 
                  onClick={() => handlePackOpen(starterPack.id)}
                  disabled={starterPack.count <= 0}
                  className={`px-4 py-2 rounded-lg font-bold transition text-sm ${starterPack.count > 0 ? `${neonBg}/30 ${neon} border border-neon hover:bg-neon/50` : 'bg-gray-700/50 text-gray-400 cursor-not-allowed'}`}
                >
                    Simulate Open
                </button>
            </div>
        ...
        <div className={`${cardBg} ${borderColor} border p-4 rounded-xl space-y-4 shadow-lg`}>
            ...
            <div className="grid grid-cols-3 text-center text-sm font-mono pt-2 border-t border-gray-800/50">
                <div className="p-2 border-r border-gray-800">
                  <p className="text-3xl font-bold text-white">{fragments}</p>
                  <p className="text-xs text-gray-400">Fragments</p>
                </div>
                <div className="p-2 border-r border-gray-800">
                  <p className={`text-3xl font-bold ${accentBlue}`}>{shards}</p>
                  <p className="text-xs text-gray-400">Shards</p>
                </div>
                <div className="p-2">
                  <p className={`text-3xl font-bold text-red-400`}>{relics}</p>
            ...
            <button 
              ...
              className={`w-full px-4 py-2 rounded-lg font-bold transition text-sm ${fragments >= 3 ? 'bg-blue-600/30 text-blue-400 border border-blue-600 hover:bg-blue-600/50' : 'bg-gray-700/50 text-gray-400 cursor-not-allowed'}`}
            >
  ...
  const QuestsTab = () => {
    ...
    const getStatusStyle = (status) => {
      switch(status) {
        ...
        case 'Locked': default: return 'bg-gray-700/50 text-gray-400 border-gray-600';
      }
    };

    return (
      <div className="space-y-6">
        <h2 className={`text-2xl font-extrabold ${neon}`}>Quests Platform</h2>
                ...
                <div key={index} className={`${cardBg} ${borderColor} border p-3 rounded-xl flex justify-between items-center shadow-md`}>
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-gray-400 flex items-center">
                            <Star className="w-3 h-3 mr-1 text-yellow-400 fill-yellow-400/50"/> {quest.reward} XP Reward
                        </p>
                    </div>
                    <button 
                        onClick={() => quest.status === 'Claimable' && handleClaim(quest.title)}
                        disabled={quest.status !== 'Claimable'}
                        className={`px-3 py-1 text-xs rounded-full font-semibold border transition ${getStatusStyle(quest.status)}`}
                    >
                        {quest.status}
                    </button>
                </div>
        ...
        <div className="space-y-3">
            <h3 className={`text-lg font-bold ${accentBlue}`}>Weekly Milestones & IRL</h3>
            {quests.filter(q => q.type !== 'Daily').map((quest, index) => (
                <div key={index} className={`${cardBg} ${borderColor} border p-3 rounded-xl flex justify-between items-center shadow-md`}>
                    <div className="space-y-1">
                        <p className="font-semibold text-white">{quest.title}</p>
  ...
  const MeshTab = () => {
    const modes = [
        ...
        { id: 'gravity', name: 'Gravity Clusters', desc: 'Visualizes clusters of wallets with the highest XP flows and token holdings.', icon: Globe }
    ];
    ...
    const legendItems = [
        ...
        { color: 'text-yellow-400', name: 'Creator Coin Flows', icon: DollarSign }
    ];
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                    {modes.map(mode => (
                        <button 
                            ...
                            className={`p-3 rounded-xl border transition duration-200 shadow-md ${activeMode.id === mode.id ? `border-neon ${neonBg}/30` : `${borderColor} bg-gray-900 hover:border-gray-500`}`}
                        >
                            <mode.icon className={`w-5 h-5 mx-auto mb-1 ${activeMode.id === mode.id ? neon : 'text-gray-400'}`} />
                ...
                <div className="text-sm text-gray-500 p-3 border-l-4 border-neon/50 bg-gray-900/50 rounded-r-xl">
                    <span className="font-semibold text-white mr-1">{activeMode.name}:</span>
                    {activeMode.desc}
                </div>
            </div>
      ...
      <div className="space-y-6">
          ...
          <div className={`${cardBg} ${borderColor} border p-4 rounded-xl text-center space-y-2 shadow-lg`}>
              ...
              <div className="flex justify-around text-center mt-2">
                  <div>
                      <p className={`text-3xl font-extrabold ${neon}`}>{userProfile.xp}</p>
                      <p className="text-xs text-gray-400">Support XP</p>
                  </div>
                  <div>
                      <p className={`text-3xl font-extrabold ${accentBlue}`}>{userProfile.solved}</p>
                      <p className="text-xs text-gray-400">Solved Cases</p>
                  </div>
                  <div>
                      <p className={`text-3xl font-extrabold text-yellow-400`}>{userProfile.rating}</p>
  ...
  const NavItem = ({ icon: Icon, tabName, label }) => (
    <button 
      ...
      className={`flex flex-col items-center p-2 rounded-lg transition duration-200 
        ${currentTab === tabName ? neon : 'text-gray-500 hover:text-white'}
      `}
    >
  ...
  const SheetAccount = () => (
    <Sheet id="account" title="Account & Rewards">
        <div className={`${cardBg} ${borderColor} border p-4 rounded-xl flex items-center space-x-4 mb-6 shadow-lg`}>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
            <div className={`${cardBg} ${borderColor} border p-3 rounded-xl shadow-md`}>
                <p className="text-sm text-gray-400">Total XP</p>
                <p className={`text-2xl font-bold ${neon}`}>{profileData.xpBalance.toLocaleString()}</p>
            </div>
            <div className={`${cardBg} ${borderColor} border p-3 rounded-xl shadow-md`}>
                <p className="text-sm text-gray-400">SPN Balance</p>
                <p className={`text-2xl font-bold ${accentBlue}`}>{profileData.spawnTokenBalance.toFixed(2)}</p>
            </div>
        </div>

        <div className={`${cardBg} ${borderColor} border p-4 rounded-xl space-y-3 shadow-lg`}>
            <h3 className={`text-lg font-semibold ${accentBlue}`}>Referral System (Pillar 9)</h3>
            <p className="text-sm text-gray-400">Share your unique code for XP bonuses and Mesh earnings.</p>
            <div className="flex justify-between items-center bg-gray-800 p-3 rounded-lg border border-gray-700">
                <span className={`font-mono ${neon} text-sm`}>SPAWN-MESH-74F</span>
                <button className={`text-xs px-3 py-1 rounded-full font-semibold ${neonBg}/30 ${neon} border border-neon hover:bg-neon/50 transition`}>Copy</button>
            </div>
        </div>
        ...
        <button className="w-full py-3 mt-8 bg-red-700/50 rounded-xl font-semibold border border-red-700 text-white/80 hover:bg-red-700/70 transition flex items-center justify-center">
            <LogOut className="w-5 h-5 mr-2"/> Switch Wallet / Logout
        </button>
    </Sheet>
  );
  ...
  const SheetSettings = () => (
    <Sheet id="settings" title="Settings & API">
        <div className="space-y-4">
            <div className={`${cardBg} ${borderColor} border p-4 rounded-xl space-y-2 shadow-lg`}>
                <h3 className={`text-lg font-semibold ${accentBlue}`}>XP SDK & Integration (Pillar 1)</h3>
                <p className="text-sm text-gray-400">Manage API keys to integrate SpawnEngine XP into your own apps.</p>
                <button className={`w-full py-2.5 ${neonBg}/30 rounded-xl font-semibold ${neon} border border-neon hover:bg-neon/50 transition text-sm`}>Show API Key</button>
            </div>

            <div className={`${cardBg} ${borderColor} border p-4 rounded-xl space-y-2 shadow-lg`}>
                <h3 className={`text-lg font-semibold ${accentBlue}`}>Premium Mesh Filters (Pillar 4)</h3>
                <p className="text-sm text-gray-400">Unlock Alpha Hunters and Whale Tracking. Requires 500 SPN staking.</p>
                <button className="w-full py-2.5 bg-gray-700/50 rounded-xl font-semibold text-gray-400 border border-gray-700/50 cursor-not-allowed text-sm">Upgrade to Premium</button>
            </div>

            <div className={`${cardBg} ${borderColor} border p-4 rounded-xl space-y-2 shadow-lg`}>
                <h3 className={`text-lg font-semibold ${accentBlue}`}>Launchpad Builder (Pillar 8)</h3>
                <p className="text-sm text-gray-400">Access the Zero-Code Token/NFT Builder and Bonding Curve configuration.</p>
  ...
  const renderTabContent = () => {
    ...
    switch (currentTab) {
      ...
      case 'loot': return <LootTab />;
      case 'quests': return <QuestsTab />;
      case 'mesh': return <MeshTab />;
        ...
        <header className="mb-6 space-y-3">
            <div className="flex justify-between items-center">
                <button onClick={() => setActiveSheet('account')} className={`flex items-center space-x-3 p-2 rounded-full ${cardBg} border ${borderColor} transition duration-200 hover:border-neon`}>
                    ...
                    <div>
                        <div className="text-base font-semibold text-white">@spawniz</div>
                        <div className="text-xs text-gray-400">Mesh ID · Creator</div>
                    </div>
                </button>
                ...
                <div className="flex space-x-2 items-center">
                    ...
                    <button onClick={() => setActiveSheet('settings')} className="text-gray-400 hover:text-white transition duration-200 p-2 rounded-full hover:bg-gray-800">
            ...
            <div className="flex flex-col space-y-2">
                <div className="flex space-x-1.5">
                    ...
                    <span className={`text-xs px-2.5 py-1 rounded-full bg-blue-600/20 text-blue-400 border border-blue-600/50`}>Farcaster Ready</span>
                </div>
                <div className="flex justify-between text-xs font-mono text-gray-500 border-t border-b border-gray-800 py-1.5 px-1">
                    <span>GAS: <span className="text-white">0.05 Gwei</span></span>
                    <span>MODE: <span className="text-white">Alpha Hunter</span></span>
                    <span>WALLETS: <span className="text-white">420k+</span></span>
                </div>
            </div>
        </header>
